...
public class Sport2SocialNetworkUserImpl<U extends User> extends SocialNetworkUserImpl<U> {
    ...
    /**
     * Returns the set of individual sports followed/practiced by this user: a sport
     * is individual if the number of team member is = 1.
     * 
	...
	for (Sport elem : this.followedSports) {
     ...
     * 
     * @param p the place in which the sport is practiced in order to be included in
     *          the resulting set
     * 
     * @return the set of sport practiced in a given place
     */
    public Set<Sport> getSportPracticedInPlace(Place p) {
	Set<Sport> placeSports = new HashSet<>();
	for (Sport elem : this.followedSports) {
	    if (elem.getPlace()==p) {
		placeSports.add(elem);
	    }
	}
	return placeSports;
    }
